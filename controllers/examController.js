  ...
  try {
    ...
    for (const exam of exams) {
      ...
      if (half === 1) {
        ...
        doc.moveTo(40, 420).lineTo(555, 420).lineWidth(1).dash(5, { space: 5 }).strokeColor('#94a3b8').stroke().undash();
      }

      // Header block
      doc.fillColor('#1e293b').font('Helvetica-Bold').fontSize(11.5).text(school?.name || 'School Name', 40, startY, { align: 'center' });
      doc.fillColor('#4f46e5').fontSize(7.5).font('Helvetica-Bold').text('EXAMINATION TIMETABLE', { align: 'center', characterSpacing: 1 });
      doc.moveDown(0.25);
      doc.fillColor('#475569').fontSize(9.5).font('Helvetica').text(`${exam_name}  ·  Class: ${exam.class_name}`, { align: 'center' });
      doc.moveDown(0.6);
      ...
      // Write table header titles in white
      doc.fillColor('#ffffff').font('Helvetica-Bold').fontSize(8.5);
      doc.text('Subject', colSubject, tableTop + 6);
      doc.text('Date', colDate, tableTop + 6);
      doc.text('Time', colTime, tableTop + 6);
      doc.text('Invigilator', colInvigilator, tableTop + 6);
      ...
      examSubjects.forEach((s, idx) => {
        ...
        if (idx % 2 === 1) {
          doc.rect(40, y, 515, rowHeight).fillColor('#f8fafc').fill();
        } else {
          doc.rect(40, y, 515, rowHeight).fillColor('#ffffff').fill();
        }
        ...
        doc.text(s.name, colSubject, y + 7, { width: 160 });
        doc.text(dateStr, colDate, y + 7);
        doc.text(timeStr, colTime, y + 7);

        if (s.invigilator_name) {
          doc.fillColor('#0f766e').font('Helvetica-Bold').fontSize(8).text(s.invigilator_name, colInvigilator, y + 7, { width: 110 });
        } else {
          doc.fillColor('#64748b').font('Helvetica').fontSize(8.5).text('TBA', colInvigilator, y + 7);
        }
    ...
    if (classIndex === 0) {
      ...
      for (let i = range.start; i < range.start + range.count; i++) {
        ...
        doc
          ...
          .fillColor('#94a3b8')
          .text(
            `Generated by EduCore • Page ${i + 1 - range.start} of ${range.count}`,
            50,
            doc.page.height - 25,
